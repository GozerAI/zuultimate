...
class TenantResponse(BaseModel):
    ...
    home_region: str = Field(default="us", description="Primary hosting region for tenant data")
    sovereignty_ring: str = Field(default="us", description="Data sovereignty ring for PII constraints")
    namespace: str = Field(default="consumer", description="Tenant namespace (consumer / enterprise)")
...
class ApiKeyCreateRequest(BaseModel):
    name: str = Field(min_length=1, max_length=255, description="Human-readable key name", examples=["Production API"])
...
class ApiKeyResponse(BaseModel):
    ...
    last_used_at: str | None = Field(default=None, description="Last usage timestamp (ISO 8601)")


class ApiKeyCreateResponse(ApiKeyResponse):
    raw_key: str = Field(..., description="Full plaintext API key (shown only once)")
...
class PasskeyRegisterBeginResponse(BaseModel):
    ...
    challenge: str = Field(..., description="Challenge identifier for completion")


class PasskeyRegisterCompleteRequest(BaseModel):
    credential: dict = Field(..., description="AuthenticatorAttestationResponse")
    challenge: str = Field(..., description="Challenge from begin step")


class PasskeyAuthBeginRequest(BaseModel):
    username: str = Field(default="", description="Optional username hint")
...
class PasskeyAuthCompleteRequest(BaseModel):
    credential: dict = Field(..., description="AuthenticatorAssertionResponse")
    challenge: str = Field(..., description="Challenge from begin step")
