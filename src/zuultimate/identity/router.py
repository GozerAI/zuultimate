...
from zuultimate.common.metrics import AUTH_REQUESTS_TOTAL, AUTH_DURATION_SECONDS, TOKEN_INTROSPECT_TOTAL
...
async def login(body: LoginRequest, request: Request):
    ...
    if is_locked:
        AUTH_REQUESTS_TOTAL.labels(method="password", status="locked_out").inc()
        raise HTTPException(
            status_code=429,
    ...
    try:
        ...
        AUTH_REQUESTS_TOTAL.labels(method="password", status="failure").inc()
        AUTH_DURATION_SECONDS.observe(time.monotonic() - start)
    ...
    if redis is not None:
        ...
        if risk.action == RiskAction.block:
            ...
            AUTH_REQUESTS_TOTAL.labels(method="password", status="blocked").inc()
            AUTH_DURATION_SECONDS.observe(time.monotonic() - start)
            raise HTTPException(
                status_code=403,
...
async def introspect_token(body: IntrospectRequest, request: Request):
    ...
    TOKEN_INTROSPECT_TOTAL.labels(status="active" if result.get("active") else "inactive").inc()
