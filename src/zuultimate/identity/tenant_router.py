...
async def list_tenants(
    ...
    svc = _get_service(request)
    if _user.get("tenant_id") is not None:
        # Non-global users can only see their own tenant
        try:
            tenant = await svc.get_tenant(_user["tenant_id"])
            return [tenant]
        except ZuulError:
            return []
...
async def get_tenant(
    ...
    if _user.get("tenant_id") is not None and _user["tenant_id"] != tenant_id:
