...
class IdentityService:
    ...
    async def _get_signing_args(self) -> dict:
        """Return kwargs for create_jwt: private_key+kid if RS256, else empty."""
        if self.key_manager is not None:
            pem, kid = await self.key_manager.get_signing_key()
            return {"private_key": pem, "kid": kid}
        return {}

    async def _get_public_keys(self) -> dict[str, str] | None:
        """Return public keys for decode_jwt if RS256, else None."""
        if self.key_manager is not None:
            return await self.key_manager.get_verification_keys()
        return None

    async def _make_token_pair(
        self, user: User, session_store=None
    ) -> tuple[str, str]:
        """Create access + refresh token pair.

        If a ``RedisSessionStore`` is provided the current generation counter
        is embedded in both tokens and the access-token session is cached in
        Redis so the hot auth path never touches the database.
        """
        # Fetch generation counter from Redis (0 when unavailable)
        gen = 0
        if session_store is not None:
            try:
                gen = await session_store.get_generation(user.id)
            except Exception:
                gen = 0
        ...
        # Cache access-token session in Redis
        if session_store is not None:
            try:
                jti = decode_jwt(access, self.settings.secret_key).get("jti")
                if jti:
                ...
                pass  # Redis unavailable — DB session is the fallback
    ...
    async def issue_tokens_for_user(self, user_id: str) -> dict:
        ...
        async with self.db.get_session(_DB_KEY) as session:
            ...
            access_token, refresh_token = await self._make_token_pair(user)
    ...
    async def login(self, username: str, password: str) -> dict:
        async with self.db.get_session(_DB_KEY) as session:
            ...
            mfa_svc = MFAService(self.db, self.settings, key_manager=self.key_manager)
    ...
    async def refresh_token(self, refresh_token: str) -> dict:
        ...
        try:
            payload = decode_jwt(refresh_token, self.settings.secret_key, public_keys=public_keys)
        except Exception:
        ...
        async with self.db.get_session(_DB_KEY) as session:
            ...
            new_access, new_refresh = await self._make_token_pair(user)
    ...
    async def introspect_token(self, token: str) -> dict:
        ...
        public_keys = await self._get_public_keys()
        try:
            payload = decode_jwt(token, self.settings.secret_key, public_keys=public_keys)
        except Exception:
            return {"active": False}
