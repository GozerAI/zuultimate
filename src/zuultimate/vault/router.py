...
async def encrypt(body: EncryptRequest, request: Request, user: dict = Depends(get_current_user)):
    svc = _get_service(request)
    try:
        return await svc.encrypt(
            plaintext=body.plaintext, label=body.label, owner_id=user["user_id"] or ""
        )
...
async def decrypt(body: DecryptRequest, request: Request, user: dict = Depends(get_current_user)):
    svc = _get_service(request)
    try:
        return await svc.decrypt(blob_id=body.blob_id, owner_id=user["user_id"] or "")
    except ZuulError as e:
        raise HTTPException(status_code=e.status_code, detail=e.message)
...
async def rotate_blob(blob_id: str, request: Request, user: dict = Depends(get_current_user)):
    svc = _get_service(request)
    try:
        return await svc.rotate_blob(blob_id, owner_id=user["user_id"] or "")
    except ZuulError as e:
        raise HTTPException(status_code=e.status_code, detail=e.message)
