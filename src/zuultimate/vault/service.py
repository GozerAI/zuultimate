...
class VaultService:
    ...
    async def decrypt(self, blob_id: str, owner_id: str = "") -> dict:
        async with self.db.get_session(_DB_KEY) as session:
            stmt = select(EncryptedBlob).where(EncryptedBlob.id == blob_id)
            if owner_id:
                stmt = stmt.where(EncryptedBlob.owner_id == owner_id)
    ...
    async def rotate_blob(self, blob_id: str, owner_id: str = "") -> dict:
